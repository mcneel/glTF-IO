...
namespace Export_glTF
{
  ...
  class RhinoMaterialGltfConverter
  {
    ...
    public int AddMaterial()
    {
      ...
      {
        transmission.TransmissionFactor = (float)pbr.Opacity;
      }
    ...
    glTFExtensions.KHR_texture_transform GetTextureTransform(Rhino.Render.RenderTexture renderTexture)
    {
      Rhino.Geometry.Vector3d offsetVector = renderTexture.GetOffset();
      Rhino.Geometry.Vector3d rotationVector = renderTexture.GetRotation();
      Rhino.Geometry.Vector3d repeatVector = renderTexture.GetRepeat();

      //No texture transform
      if(offsetVector == Rhino.Geometry.Vector3d.Zero &&
        rotationVector == Rhino.Geometry.Vector3d.Zero &&
        repeatVector == new Rhino.Geometry.Vector3d(1.0, 1.0, 1.0))
      {
        return null;
      }

      Rhino.Geometry.Plane rhinoBasePlane = new Rhino.Geometry.Plane(new Rhino.Geometry.Point3d(0.0, 1.0, 0.0), Rhino.Geometry.Vector3d.ZAxis);

      //glTF Y is reversed
      Rhino.Geometry.Transform t = Rhino.Geometry.Transform.Translation(new Rhino.Geometry.Vector3d(offsetVector.X, -offsetVector.Y, 0.0));

      //Only need the Z rotation
      Rhino.Geometry.Transform r = Rhino.Geometry.Transform.Rotation(rotationVector.Z, rhinoBasePlane.Normal, rhinoBasePlane.Origin);

      //Only need X, Y repeat
      Rhino.Geometry.Transform s = Rhino.Geometry.Transform.Scale(rhinoBasePlane, repeatVector.X, repeatVector.Y, 1.0);

      Rhino.Geometry.Transform compiled = t * r * s;

      //Decompose to glTF
      compiled.DecomposeTextureMapping(out Rhino.Geometry.Vector3d translation, out Rhino.Geometry.Vector3d scale, out Rhino.Geometry.Vector3d rotation);

      if (
        rotation == Rhino.Geometry.Vector3d.Unset ||
        scale == Rhino.Geometry.Vector3d.Unset
        )
      {
      ...
      textureTransform.Rotation = (float)rotation.Z;
